...
This is my attempt to summarize and share the theory that William Calvin laid out in "The Cerebral Code". If you haven't read it and are interested in how how the brain works or AI/ML you should order a copy right now. I'm going to do the book no justice but hopefully, if you haven't already ordered the book, the rest of this post will convince you to read it.
...
If you find these ideas compelling, please share this article.
...
He decided the only way that life was possible was if it lead to more entropy throughout the system. Like a tornado, life is a pocket of order that sows disorder. It lives so long as it pays the entropy tax.

To anthropomorphize a bit, The Universe is trying to maximize entropy but it has a near infinite "problem space". One algorithm it has found to solve this problem is evolution. To butcher a CS phrase, I believe that evolution is the "best case runtime" when trying to find increasingly valid solutions to a near infinite problem space.

Essentially, biology uses evolution because it is the best way to solve the problem of prediction in a complex world.
...
David Eagleman showed that everything else in the brain is also massively parallel. He dubbed this the "Mr. Potato Head" theory because no matter where you "plug in" sensory input, your brain tends to figure out what to do with it. Plug some eyes into the olfactory cortex and a nose into the visual cortex and the brain will keep right on trucking.
...
To prevent confusion, I'm saying the recipe for more "animal intelligence" is adding more columns. The recipe for "human intelligence" is "animal intelligence" plus high fidelity imitation. Something I'll write about in a future post.
...
Before we go deeper lets pause and notice that this theory satisfies all our constraints. It implements the "best case runtime" algorithm for predicting "valid" solutions to a complex problem space. It is massively parallel with computation and storage happening between and within individual minicolumns. The calculation is done with a spatiotemporal "Morse Code" message. But storage is performed by changing the spatial relationships within a minicolumn so that the message is more easily triggered in the future.

There is so much deeper to go there and if you're interested I recommend the book. We're going to keep moving.
...
Looking down on the brain again, we can imagine laying down a fishing net of equilateral triangles. Each vertex in the net will land on a minicolumn within the same network, leaving holes over minicolumns that don't belong to that network. If we were to lay nets over the network until every minicolumn was covered by a vertex we would lay 50-100 nets.
...
Or in the other direction. When you're thinking hard about a question, you could let your mind wander a little (isn't that when all the good ideas hit us?). Maybe the act of mind wandering lets some message within the cortical column get exchanged for another. And WAM! A new idea, a possible solution.
...
I would love to help build this but I don't have much background in AI/ML. If you or anyone you know would be interested talking about this, trying to build this, or mentoring me as I try, please reach out. Again, my email is calepayson(at)mac.com.

Heck if you're reading this from some large AI lab and might run some experiments on this, I'll move out to wherever you are for the chance to help. Even if it just means keeping the lab clean (though I hope I can add more than that).
